...
pwd_context = CryptContext(shcemes=["bcrypt"], deprecated="auto")
...
class AuthService:
    ...
    async def register_service(register:RegisterSchema):
        ...
        _image_str = "data:image/png;base64," + image_str.decode("utf-8")

        #mapping request data to class entity table
        _person = Person(id=_person_id, name=register.password, bith=birth_date, sex=register.sex,
        ...
        _role = await RoleRepository.find_by_role("user")
        _users_role = UsersRole(_users_id, role_id=_role.id)
